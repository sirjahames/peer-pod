...
function computeBigFiveCompatibility(scores1: BigFiveScores, scores2: BigFiveScores): number {
    ...
    const conscientiousnessMatch =
        100 - Math.abs(scores1.conscientiousness - scores2.conscientiousness);
    ...
    const extraversionComplement =
        50 + (100 - Math.abs(scores1.extraversion + scores2.extraversion - 100)) / 2;
...
function computeWorkStyleCompatibility(ws1: WorkStyleAssessment, ws2: WorkStyleAssessment): number {
    ...
    const gradeDiff = Math.abs(
        gradeOrder.indexOf(ws1.gradeExpectation) - gradeOrder.indexOf(ws2.gradeExpectation),
    );
    ...
    const deadlineDiff = Math.abs(
        deadlineOrder.indexOf(ws1.deadlineStyle) - deadlineOrder.indexOf(ws2.deadlineStyle),
    );
    ...
    else if (ws1.teamRole === ws2.teamRole)
        score += 8; // Same role is less ideal
    else score += 14;
    ...
    else if (ws1.missingWorkResponse === "checkIn" || ws2.missingWorkResponse === "checkIn")
        score += 10;
    else score += 5;
 ...
 * Returns a score from 0 to 100
 */
function computeSchedulingCompatibility(
    sched1: SchedulingAssessment,
    sched2: SchedulingAssessment,
): number {
    let score = 0;
    ...
    const responseDiff = Math.abs(
        responseOrder.indexOf(sched1.responseTime) - responseOrder.indexOf(sched2.responseTime),
    );
    ...
    else if (sched1.meetingFormat === "hybrid" || sched2.meetingFormat === "hybrid") score += 10;
    else if (
    ...
    const days: (keyof AvailabilityGrid)[] = [
        "monday",
        "tuesday",
        "wednesday",
        "thursday",
    ...
    else if (sched1.flexibility === "very" || sched2.flexibility === "very") score += 10;
    else score += 5;

    // Commitment compatibility (busy people should work with busy people)
    const busy1 = Object.values(sched1.commitments).filter(
        (v) => v && !sched1.commitments.scheduleClear,
    ).length;
    const busy2 = Object.values(sched2.commitments).filter(
        (v) => v && !sched2.commitments.scheduleClear,
    ).length;
...
export function computeProjectCompatibility(profile: FreelancerProfile, project: Project): number {
    ...
    if (profile.quizResult?.bigFiveScores) {
        ...
        const personalityScore = Math.round(
            (bf.conscientiousness || 50) * 0.4 +
                (100 - (bf.neuroticism || 50)) * 0.3 +
                (bf.openness || 50) * 0.2 +
                (bf.agreeableness || 50) * 0.1,
        );
    ...
    const personalityLength = personality.length > 0 ? personality.length : 9;
    ...
    return isNaN(result) ? 50 : Math.min(100, Math.max(0, result));
}
...
export function computeFreelancerCompatibility(
    ...
    if (hasQuizData1 && hasQuizData2) {
        ...
        const skills1Set = profile1.skills || [];
        const skills2Set = profile2.skills || [];
        const skills1 = new Set(skills1Set.map((s) => s.skill.toLowerCase()));
        const skills2 = new Set(skills2Set.map((s) => s.skill.toLowerCase()));
    ...
    // Skill complementarity with safe defaults
    const skills1Arr = profile1.skills || [];
    const skills2Arr = profile2.skills || [];
    const skills1 = new Set(skills1Arr.map((s) => s.skill.toLowerCase()));
    const skills2 = new Set(skills2Arr.map((s) => s.skill.toLowerCase()));
    ...
    // Availability overlap with safe defaults
    const hours1 = profile1.availability?.hoursPerWeek || 20;
    const hours2 = profile2.availability?.hoursPerWeek || 20;
