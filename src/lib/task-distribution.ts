...
import { freelancerProfiles, generateId } from './data';
  ...
  const tasks: TaskTemplate[] = [
    {
      ...
      description: 'Set up the project structure, configure build tools, and establish coding standards.',
 ...
 */
function scoreFreelancerForTask(
...
export function assignTasksToTeam(
  ...
  const templates = generateTasksForProject(
    projectTitle,
    requiredSkills,
    memberIds.length
  );
  ...
  for (const template of templates) {
    ...
    for (const memberId of memberIds) {
      const profile = freelancerProfiles.get(memberId);
      if (!profile) continue;
