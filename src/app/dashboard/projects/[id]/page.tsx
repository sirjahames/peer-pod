...
import { Project, User, FreelancerProfile, Group, Task, GroupStatus, JobType, ExperienceLevel, PaymentType, WorkLocation } from '@/lib/types';
      ...
      <div>
        ...
        {teamTab === 'settings' && (
          <div className="max-w-2xl">
            ...
            <div className="p-6 border rounded-lg mb-6">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  <p className="font-medium">{formatExperience(project.experienceLevel)}</p>
                </div>
                <div>
                  <span className="text-gray-600">Payment</span>
                  <p className="font-medium text-green-700">{formatPayment(project)}</p>
                </div>
                <div>
            ...
            {(project.responsibilities?.length || project.requirements?.length || project.benefits?.length) && (
            ...
            <div className="p-6 border border-red-200 rounded-lg bg-red-50">
              ...
              <p className="text-sm text-red-600 mb-4">
                Deleting this project will remove all associated data including the team and tasks.
              </p>
              <button
      ...
      {!isEditing && (
        <>
          <div className="flex flex-wrap gap-4 mb-4 text-sm text-gray-600">
            ...
            <span>Join code: <strong>{project.joinCode}</strong></span>
          </div>
                      ...
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <p className="text-sm text-gray-600 mt-2">
                            {candidate.profile.availability.hoursPerWeek} hrs/week •{' '}
                ...
                <div className="space-y-3 mb-6">
                  {suggestions.map((suggestion, idx) => {
                    const memberNames = suggestion.members
                      .map(
                        (memberId) => candidates.find((c) => c.user.id === memberId)?.user.name
                      )
                      .filter(Boolean);
                ...
                <div className="border-t pt-4">
                  ...
                  <div className="space-y-2 mb-4">
                    {selectedMembers.map((memberId) => {
                      const candidate = candidates.find((c) => c.user.id === memberId);
                      return (
                        <div
                          key={memberId}
                          className="flex justify-between items-center p-2 bg-gray-100 rounded"
                        >
                          <span className="text-sm">{candidate?.user.name}</span>
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
