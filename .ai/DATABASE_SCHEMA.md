...
### Database Trigger
A trigger automatically creates user profiles when someone signs up via Supabase Auth:
...
- Freelancer profiles are publicly viewable (for matching)
- Project owners can manage their projects and view applications
- Group members can view tasks/messages in their groups
- Data access is enforced at the database level

**Setup files:**
- `supabase/schema.sql` - Table definitions
- `supabase/auth-setup.sql` - RLS policies and auth trigger